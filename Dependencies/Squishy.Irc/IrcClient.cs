...
namespace Squishy.Irc
{
	...
	public class IrcClient
	{
		...
		public void BeginConnect(string addr, IPEndPoint bindAddr = null)
		{
			BeginConnect(addr, IrcProtocol.DefaultPort, bindAddr);
		}
		...
		public void BeginConnect(string addr, int port, IPEndPoint bindAddr = null)
		{
			m_me.SetInfo(m_nicks[0], UserName, ServerPassword, Info);
			m_client.BeginConnect(addr, port, bindAddr);
		}
		...
		{
		    return (from ircUser in m_Users where ircUser.Value.Nick.ToLower() == nick.ToLower() select ircUser.Value).FirstOrDefault();
		}
		...
		protected virtual void OnConnectionInfo(IrcPacket packet, Dictionary<string, string> pairs)
		{
			...
			else if ("freenode".Equals(Network.Name, StringComparison.InvariantCultureIgnoreCase))
			{
                ...
                AuthMgr.Authenticator = new NickServAuthenticator("330");
		}
		...
		/// <summary>
		/// Fires when the Client receives any PRIVMSG or NOTICE excluding CTCP requests.
		/// </summary>
		...
		/// </summary>
		protected virtual void OnInvite(IrcUser user, IrcChannel chan)
		{
		}

		internal void InviteNotify(IrcUser user,string chan)
		{
		    var channel = GetOrCreateChannel(chan);
		...
		/// </summary>
		protected virtual void OnUnknownCommandUsed(CmdTrigger<IrcCmdArgs> trigger)
		{
			trigger.Reply("Use " + CommandHandler.RemoteCommandPrefixes + "Help for an overview over all available commands!");
			//Send(trigger.Args.String);
		}

		internal void UnkownCommandUsedNotify(CmdTrigger<IrcCmdArgs> trigger)
		{
		...
		/// </summary>
		protected virtual void OnCommandFail(CmdTrigger<IrcCmdArgs> trigger, Exception ex)
		{
		}

		internal void CommandFailNotify(CmdTrigger<IrcCmdArgs> trigger, Exception ex)
		{
